    ...
    this.subscriptions.add(
      atom.workspace.addOpener((uriToOpen) => {
        ...
        if (protocol === 'pulsar-jupyter-notebook:' || pathname?.endsWith('.ipynb')) {
          const editor = new NotebookEditor(uriToOpen);
          const view = new NotebookEditorView({ notebookEditor: editor });

          // Mount React component to editor element
          const React = require('react');
          const ReactDOM = require('react-dom');
          ReactDOM.render(React.createElement(NotebookEditorView, { notebookEditor: editor }), editor.element);
